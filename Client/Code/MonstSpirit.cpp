...
CMonstSpirit::CMonstSpirit(const CMonstSpirit& rhs)
	...
	m_pBlueOrbTransCom(rhs.m_pBlueOrbTransCom)
{
}
...
_int CMonstSpirit::Update_Object(const _float& fTimeDelta)
{
	...
	// 파란구
	m_pBlueOrbTransCom->Set_Scale(_vec3{ m_fBlueOrbSize, m_fBlueOrbSize, m_fBlueOrbSize });
	m_pBlueOrbTransCom->Set_Pos(m_vBlueOrbPos);
...
void CMonstSpirit::Render_Object()
{
	...
	m_pGraphicDev->SetTransform(D3DTS_WORLD, &m_pBlueOrbTransCom->Get_WorldMat());
	m_pTexBlueOrbCom->Render_Texture();
	m_pBufferCom->Render_Buffer();
...
HRESULT CMonstSpirit::Add_Component()
{
	...
	// 파란구
	pComponent = m_pBlueOrbTransCom = dynamic_cast<CTransform*>(CProtoMgr::GetInstance()->Clone_Proto(COMPONENT_TYPE::TRANSFORM, this));
	NULL_CHECK_RETURN(pComponent, E_FAIL);
	m_mapComponent[ID_DYNAMIC].emplace(COMPONENT_TYPE::TRANSFORM, pComponent);

	pComponent = m_pTexBlueOrbCom = dynamic_cast<CTexture*>(Engine::Clone_Texture(L"Proto_Texture_Effect_Spiritorb", this));
	NULL_CHECK_RETURN(pComponent, E_FAIL);
	m_mapComponent[ID_STATIC].emplace(COMPONENT_TYPE::TEXTURE, pComponent);
