...
namespace Engine
{
	...
	enum class OBJ_TYPE {
		PLAYER, MONSTER, NPC, ITEM, PROJECTILE, CAMERA,
		ENVIORMENT, TERRAIN, BACKGROUND, UI, EFFECT, TYPEEND
	}; // 오브젝트 타입

	enum class COMPONENT_TYPE {
		TRANSFORM, COL_RECT, COL_CIRCLE, COL_LINE, ANIMATOR, STATEMACHINE,
		BUFFER_RC_TEX, BUFFER_RC_COL, BUFFER_CUBE, BUFFER_TERRAIN_TEX,
		TEXTURE, CAMERA, TYPEEND
	}; // 컴포넌트 종류
	...
	enum TILE_TAG {
		TILE_GRASS_FIELD, TILE_WATER_FIELD, TILE_ICE_FIELD, TILE_ICE_DUNGEON, TILE_STONE_DUNGEON,
		TILE_STONE_DUNGEON2, TILE_TEMPLE_DUNGEON, TILE_SMITH, TILE_END
	}; 	// IMGUI 타일 폴더 경로 불러오기 위한 타일 종류 분류
